...
class CmiBoundSimulator:
    ...
    def train(self, num_epochs=1300):
        ...
        for epoch in range(num_epochs):
            ...
            if epoch % 100 == 0:
                template = 'Epoch {}, Accuracy train: {}, arg_theta: {}, LR: {}, var: {}, cmi_bound: {}'
...
class GradBoundSimulator:
    def __init__(self):
        self.num_traindp = 20000  # it is not 50000 so that we can estimate the impact of different datasets
        self.x_train, self.y_train, self.x_train_tensor, self.y_train_tensor, self.test_ds = self.fmnist_ds()
        self.model = self.model_def()
        self.loss_fn = tf.keras.losses.CategoricalCrossentropy(from_logits=True)
        self.train_loss = tf.keras.metrics.Mean(name='train_loss')
        self.train_accuracy = tf.keras.metrics.CategoricalAccuracy(name='train_accuracy')
