...
class CmiBoundSimulator:
    def __init__(self):
        ...
        self.num_traindp = 15000  # it is not 50000 so that we can estimate the impact of different datasets
    ...
    def model_def(self):  # model definition
        model = tf.keras.models.Sequential([
            tf.keras.layers.Conv2D(32, (3, 3), activation=tf.nn.relu, input_shape=(32, 32, 3),
                                   kernel_initializer='he_uniform'),
            tf.keras.layers.MaxPool2D(pool_size=(2, 2)),
            tf.keras.layers.Conv2D(64, (3, 3), activation=tf.nn.relu, kernel_initializer='he_uniform'),
            tf.keras.layers.Conv2D(32, (3, 3), activation=tf.nn.relu, kernel_initializer='he_uniform'),
            tf.keras.layers.MaxPool2D(pool_size=(2, 2)),
    ...
    def train(self, num_epochs=2300):
        ...
        for epoch in range(num_epochs):
            ...
            if epoch % 100 == 0:
                template = 'Epoch {}, Loss: {}, Accuracy train: {}, Accuracy test: {}, arg_theta: {}, cmi_bound: {}'
...
class GradBoundSimulator:
    def __init__(self):
        self.num_traindp = 15000  # it is not 50000 so that we can estimate the impact of different datasets
        self.x_train, self.y_train, self.x_train_tensor, self.y_train_tensor, self.test_ds = self.cifar_ds()
        self.model = self.model_def()
        self.loss_fn = tf.keras.losses.CategoricalCrossentropy(from_logits=True)
        self.train_loss = tf.keras.metrics.Mean(name='train_loss')
        self.train_accuracy = tf.keras.metrics.CategoricalAccuracy(name='train_accuracy')
...
class IncohBoundSimulator:
    ...
    @tf.function
    def training_step(self, var_n, lr, gradients_):  
    ...
    def train(self, num_epochs=2300):
        ...
        for epoch in range(num_epochs):
            ...
            beta_t = 2 * lr_t / var_noise_
            beta_t_tensor = tf.convert_to_tensor(beta_t, dtype=tf.float32)
            grad_list = self.compute_gradients()  
